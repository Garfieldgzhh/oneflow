...
class ReduceGlobal : public OpExprGradFunction<ReduceGlobalCaptureState> {
 public:
  ...
  Maybe<void> Apply(const ReduceGlobalCaptureState* ctx, const TensorTuple& out_grads,
    ...
    if(mode == ReduceMode::kMin) {
        in_grads->at(0) =
            JUST(functional::ReduceMinGlobalStageGrad(out_grads.at(0), mask, device_count, ctx->axis, ctx->keepdims));
    } else {
        in_grads->at(0) =
            JUST(functional::ReduceMaxGlobalStageGrad(out_grads.at(0), mask, device_count, ctx->axis, ctx->keepdims));
    }
    return Maybe<void>::Ok();
  }
