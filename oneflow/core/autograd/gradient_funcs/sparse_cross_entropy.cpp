...
class DistributedSparseCrossEntropy : public OpExprGradFunction<SparseCrossEntropyCaptureState> {
 public:
  ...
  Maybe<void> Apply(const SparseCrossEntropyCaptureState* ctx, const TensorTuple& out_grads,
    ...
    in_grads->resize(2);
    in_grads->at(0) =
        JUST(functional::DistributedSparseCrossEntropyGrad(prediction, label, out_grads.at(0), ctx->depth));
    return Maybe<void>::Ok();
  }
...
REGISTER_OP_EXPR_GRAD_FUNCTION("sparse_cross_entropy_ms", DistributedSparseCrossEntropy);
